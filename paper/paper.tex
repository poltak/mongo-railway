...
\lstdefinelanguage{JavaScript}{
  keywords={typeof, new, true, false, catch, function, delete, return, undefined, null, catch, switch, var, if, in, while, do, else, case, break},
...
overview of the MongoDB database will be given in~\sectref{sec:mongo_overview}. Implementation details and
discussion will be given in~\sectref{sec:implementation} and~\sectref{sec:discussion}, respectively,
...
single-key, multi-key, geospatial, and hashed indexes. Examples of creating indexes will be given in~\sectref{sec:implementation}.
...
Further restructuring can be done to group the \textbf{Bounce}, \textbf{Rock}, \textbf{Lat}, \textbf{Lon}, and \textbf{Acc} values into sub-documents
containing key-value pairs for each, finally ending up with the following document structure:

\begin{lstlisting}[caption=Improved multi-layered document structure.]
{
...
These restructurings could be performed iteratively using the following code, in mongo shell for example:

\begin{lstlisting}[caption=Restructuring of documents in ``train'' collection using an iteratively applied operation.]
...
\subsection{Ad-hoc adding of additional sensors} % (fold)
...
where such values exist for these keys, thus ignoring documents from the original dataset, as touched on in~\sectref{sub:handling_missing_values}.
...
% subsection adding_additional_sensors (end)
...
As most of what has been shown in~\sectref{sec:implementation}, properly setting up the MongoDB database for use with the IRT
...
As noted in~\sectref{sec:mongo_overview}, MongoDB allows for much more advanced operations to be performed, however
...
A further difference that may catch many users of relational database, is the lack of joins, as noted in~\sectref{ssub:joins}.
